...
export const Checkout = () => {
  ...
  const { cartItems, orderNotes } = useContext(ShopContext);
  const [orderNumber, setOrderNumber] = useState('');
  const [orderTotal, setOrderTotal] = useState(0);
  const { deliveryMethod } = useContext(ShopContext);


  const checkout = async (orderNotes, deliveryMethod) => {
    try {
      // 将购物车商品信息传递到后端
      const orderData = {
  ...
  return (
    <div className="checkoutcontent">
      ...
      <p>
        配送方式: {deliveryMethod === '1' ? '送货' : deliveryMethod === '2' ? '自取：地點1' : '自取：地點2'}
      </p>
