...
app.post("/checkout", async (req, res) => {
  ...
  try {
    ...
    const { userID, username, productDetails, orderNotes, deliveryMethod, totalAmount } = req.body;


    // 插入订单
    const [result] = await db.execute(
      "INSERT INTO orders (orderNumber, userID, username, totalAmount, productDetails, deliveryMethod, orderNotes, createdate) VALUES (?, ?, ?, ?, ?, ?, ?, now())",
      [orderNumber, userID, username, totalAmount, productDetails, deliveryMethod, orderNotes]
    );

    // 提交事务
    await db.commit();

    res.status(200).json({ orderNumber, totalAmount, orderNotes, productDetails });
  } catch (error) {
    // 如果发生错误，回滚事务
    await db.rollback();
